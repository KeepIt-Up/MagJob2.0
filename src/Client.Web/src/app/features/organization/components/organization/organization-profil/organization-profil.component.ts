...
import { OrganizationService, UpdateOrganizationPayload } from '../../../../../apis/organization.api.service';
...
@Component({
  ...
  styleUrl: './organization-profil.component.css'
})
export class OrganizationProfilComponent implements OnInit, OnDestroy {
  ...
  constructor() {
    effect(() => {
      const org = this.organizationStore.data();
      if (org) {
  ...
  private handleImageUpload(file: File, imageType: 'profileImage' | 'bannerImage'): void {
    ...
    reader.onload = () => {
      ...
      this.organizationService.updateOrganization(
        this.organizationStore.data()?.id.toString() || '',
        payload
      ).subscribe({
        next: () => {
          this.organizationStore.loadOrganization(this.organizationId);
